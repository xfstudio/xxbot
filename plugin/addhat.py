...
from configs import LOGGER, MODELS_DIR, IMAGE_DIR
MODELS_PATH = os.path.join(MODELS_DIR, 'shape_predictor_5_face_landmarks.dat')
HAT_PATH = os.path.join(IMAGE_DIR, 'hat2.png')
HAT_ALPHA_PATH = os.path.join(IMAGE_DIR, 'hat_alpha.jpg')
HAT_ORIGIN_PATH = os.path.join(IMAGE_DIR, 'hat.jpg')
HAT_BG_PATH = os.path.join(IMAGE_DIR, 'hat_bg.jpg')
...
class Addhat():
    ...
    实例化时载入多个头照片资源
    '''
    def __init__(self,heads_list=[],predictor_path=MODELS_PATH):
        '''
        head_list:
	...
	def add_hat(img,hat_img):
	    ...
	    if len(dets)>0:  
	        for d in dets:
	            ...
	            cv2.imwrite(HAT_BG_PATH,bg)
	            # cv2.imshow("image",img)
	            # cv2.waitKey()

	            # 提取帽子区域
	            hat = cv2.bitwise_and(resized_hat,resized_hat,mask = mask)
	            cv2.imwrite(HAT_ORIGIN_PATH,hat)
	...
	def add_hat_file(in_img, hat_img=HAT_PATH):
